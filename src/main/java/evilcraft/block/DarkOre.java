...
public class DarkOre extends ConfigurableBlock implements IInformationProvider {
    ...
    public static void initInstance(ExtendedConfig<BlockConfig> eConfig) {
        if (_instance == null) _instance = new DarkOre(eConfig);
    ...
    public Item getItemDropped(int meta, Random random, int zero) {
        if (Configs.isEnabled(DarkGemConfig.class)) return DarkGem.getInstance();
        else return null;
    }
    ...
    public ArrayList<ItemStack> getDrops(World world, int x, int y, int z, int metadata, int fortune) {
        ...
        if ((fortune > 0 || world.rand.nextInt(CRUSHEDCHANCE) == 0) && Configs.isEnabled(DarkGemCrushedConfig.class)) {
            drops.add(
                new ItemStack(
                    DarkGemCrushedConfig._instance.getItemInstance(),
    ...
    @Override
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int meta, float motionX,
    ...
    private void sparkle(World world, int x, int y, int z) {
        ...
        for (int l = 0; l < 6; ++l) {
            ...
            if (sparkX < (double) x || sparkX > (double) (x + 1)
                || sparkY < 0.0D
                || sparkY > (double) (y + 1)
                || sparkZ < (double) z
                || sparkZ > (double) (z + 1)) {
    ...
    public String getInfo(ItemStack itemStack) {
        return IInformationProvider.INFO_PREFIX + L10NHelpers
            .localize(this.getUnlocalizedName() + ".info.custom", DarkOreConfig.startY, DarkOreConfig.endY);
    }
