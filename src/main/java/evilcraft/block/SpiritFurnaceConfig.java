...
public class SpiritFurnaceConfig extends BlockContainerConfig {
    ...
    @ConfigurableProperty(
        ...
        comment = "How much mB per tick this furnace should consume.")
    ...
    @ConfigurableProperty(
        category = ConfigurableTypeCategory.MACHINE,
        comment = "How much mB per tick this furnace should consume for player spirit.")
    ...
    @ConfigurableProperty(
        category = ConfigurableTypeCategory.MACHINE,
        comment = "The required amount of ticks for each HP for cooking an entity.")
    ...
    @ConfigurableProperty(
        category = ConfigurableTypeCategory.MACHINE,
        comment = "If the machine should play mob death sounds.")
    ...
    @ConfigurableProperty(
        category = ConfigurableTypeCategory.MACHINE,
        comment = "The 1/X chance for villagers to drop emeralds. 0 means no drops.")
    public static int villagerDropEmeraldChance = 20;

    /**
     * Custom player drops. Maps player UUID to an itemstack. Expects the format domain:itemname:amount:meta for items
     * where amount and meta are optional.
     */
    @ConfigurableProperty(
        category = ConfigurableTypeCategory.MACHINE,
    ...
    public static String[] playerDrops = new String[] { "93b459be-ce4f-4700-b457-c1aa91b3b687|minecraft:stone_slab", // Etho's
                                                                                                                     // Slab
    };
    ...
    public void onInit(IInitListener.Step step) {
        ...
        if (step == IInitListener.Step.INIT) {
            EvilCraft.IMC_HANDLER
                .registerAction(Reference.IMC_OVERRIDE_SPIRITFURNACE_DROPS, new IMCHandler.IIMCAction() {

                    @Override
                    public boolean handle(FMLInterModComms.IMCMessage message) {
                        if (!message.isNBTMessage()) return false;
                        try {
                            Class<EntityLivingBase> clazz = (Class<EntityLivingBase>) Class.forName(
                                message.getNBTValue()
                                    .getString("entityClass"));
                            Set<WeightedItemStack> itemStacks = Sets.newHashSet();
                            NBTTagList list = message.getNBTValue()
                                .getTagList("items", MinecraftHelpers.NBTTag_Types.NBTTagCompound.ordinal());
                            ...
                            EvilCraft
                                .log("IMC override mob drop did not provide an existing entity class.", Level.ERROR);
                            return false;
                        } catch (ClassCastException e) {
                            EvilCraft.log(
                                "IMC override mob drop did not provide an entity class of type EntityLivingBase.",
                                Level.ERROR);
                            return false;
                        }
                        return true;
                ...
                for (String line : SpiritFurnaceConfig.playerDrops) {
                    ...
                    if (split.length != 2) {
                        throw new IllegalArgumentException(
                            "Invalid line '" + line + "' found for " + "a Spirit Furnace player drop config.");
                    }
                    try {
                        ...
                        if (!validId) {
                            EvilCraft.log(
                                "Invalid line '" + line
                                    ...
                                    + split[0]
                                    + " does not refer to a valid player UUID; skipping.");
                        }
                        try {
                            ...
                            EvilCraft.log(
                                "Invalid item '" + split[1]
                                    + "' in "
                                    + "a Spirit Furnace player drop config; skipping:"
                                    + e.getMessage(),
                                Level.ERROR);
                        }
