...
public class EntangledChaliceItem extends ItemBlockFluidContainer {
    ...
    public String getTankID(ItemStack container) {
        ...
        if (container.stackTagCompound == null || !container.stackTagCompound.hasKey(key)) {
            ...
            container.stackTagCompound.getCompoundTag(key)
                .setString(WorldSharedTank.NBT_TANKID, "invalid");
        }
        return container.stackTagCompound.getCompoundTag(key)
            .getString(WorldSharedTank.NBT_TANKID);
    }
    ...
    public void setTankID(ItemStack container, String tankID) {
        ...
        container.stackTagCompound.getCompoundTag(key)
            .setString(WorldSharedTank.NBT_TANKID, tankID);
    }
        ...
        setTankID(
            ...
            Integer.toString(
                GlobalCounter.getInstance()
                    .getNext("EntangledChalice")));
    }

    @Override
    public FluidStack getFluid(ItemStack container) {
        return WorldSharedTankCache.getInstance()
            .getTankContent(getTankID(container));
    }

    @Override
    protected void setFluid(ItemStack container, FluidStack fluidStack) {
        WorldSharedTankCache.getInstance()
            .setTankContent(getTankID(container), fluidStack);
    }
