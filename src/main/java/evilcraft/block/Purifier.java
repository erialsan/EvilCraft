...
public class Purifier extends ConfigurableBlockContainer {
    ...
    public static void initInstance(ExtendedConfig<BlockConfig> eConfig) {
        if (_instance == null) _instance = new Purifier(eConfig);
    ...
    @Override
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float motionX,
        ...
        if (world.isRemote) {
            ...
            if (tile != null) {
                if (itemStack == null && tile.getPurifyItem() != null) {
                    ...
                    if (fluidStack != null && tile.getTank()
                        .canTankAccept(fluidStack.getFluid())
                        && tile.getTank()
                            .canCompletelyFill(fluidStack)
                        ...
                        if (buckets > 0) {
                            if (!player.capabilities.isCreativeMode) {
                                player.inventory.setInventorySlotContents(
                                    ...
                                    new ItemStack(BucketBloodConfig._instance.getItemInstance()));
                            }
                            tile.setBuckets(buckets - 1, tile.getBucketsRest());
                        }
                        return true;
                    }
                } else if (itemStack != null && tile.getActions()
                    .isItemValidForAdditionalSlot(itemStack) && tile.getAdditionalItem() == null) {
                        ...
                        .isItemValidForMainSlot(itemStack) && tile.getPurifyItem() == null) {
                            ItemStack copy = itemStack.copy();
                            copy.stackSize = 1;
    ...
    @Override
    public void addCollisionBoxesToList(World world, int x, int y, int z, AxisAlignedBB area, List collisionBoxes,
        Entity entity) {
    ...
    public static IIcon getPurifierIcon(String icon) {
        return icon.equals("inner") ? Purifier._instance.innerIcon
            : (icon.equals("bottom") ? Purifier._instance.bottomIcon : null);
    }
