...
public class ExcrementPile extends ConfigurableBlock implements IInformationProvider {
    ...
    private static final int CHANCE_DESPAWN = 1; // 1/CHANCE_DESPAWN per random tick chance for despawning and
                                                 // potentially triggering bonemeal event
    private static final int CHANCE_BONEMEAL = 3; // 1/CHANCE_BONEMEAL for ground below to be bonemealed or turn into
                                                  // grass from dirt
    ...
    public static void initInstance(ExtendedConfig<BlockConfig> eConfig) {
        if (_instance == null) _instance = new ExcrementPile(eConfig);
    ...
    public boolean canPlaceBlockAt(World world, int x, int y, int z) {
        ...
        return (block.isLeaves(world, x, y - 1, z) || block.isOpaqueCube()) && block.getMaterial()
            .blocksMovement();
    }

    @Override
    public boolean canHarvestBlock(EntityPlayer player, int meta) {
        return player.getCurrentEquippedItem() != null && Configs.isEnabled(BroomConfig.class)
            && player.getCurrentEquippedItem()
                .getItem() == Broom.getInstance();
    }
            ...
            if (entity instanceof EntityPig) {
                if (isChanceWithHeight(world, x, y, z)) {
                    ((EntityLivingBase) entity)
                        .addPotionEffect(new PotionEffect(Potion.heal.id, PIG_BOOST_DURATION * 20, 1));
                    ((EntityLivingBase) entity)
                        .addPotionEffect(new PotionEffect(Potion.moveSpeed.id, PIG_BOOST_DURATION * 20, 1));
                }
                ...
                if (isChanceWithHeight(world, x, y, z)) ((EntityLivingBase) entity)
