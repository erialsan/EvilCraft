...
public class CraftingRecipeAppendix extends RecipeAppendix<IRecipe> {
    ...
    @Override
    protected void drawElementInner(GuiOriginsOfDarkness gui, int x, int y, int width, int height, int page, int mx,
        int my) {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                renderItem(
                    gui,
                    x + (SLOT_SIZE + SLOT_OFFSET_X) * i,
                    y + (SLOT_SIZE + SLOT_OFFSET_Y) * j,
                    grid[i + j * 3],
                    mx,
                    my,
                    INPUT[i + j * 3]);
        ...
        renderItem(
            gui,
            x + (SLOT_SIZE + SLOT_OFFSET_X) * 3,
            y + SLOT_OFFSET_Y + SLOT_SIZE,
    ...
    protected List<ItemStack> getItemStacks(int index) {
        ...
        if (recipe instanceof ShapedRecipes) {
            itemStacks = formatShapedGrid(
                ((ShapedRecipes) recipe).recipeItems,
                ((ShapedRecipes) recipe).recipeWidth,
                ((ShapedRecipes) recipe).recipeHeight);
        } else if (recipe instanceof ShapedOreRecipe) {
            itemStacks = formatShapedGrid(
                ((ShapedOreRecipe) recipe).getInput(),
            ...
            itemStacks = ((ShapelessOreRecipe) recipe).getInput()
                .toArray();
        } else {
