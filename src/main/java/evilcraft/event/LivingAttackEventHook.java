...
public class LivingAttackEventHook {
    ...
    private void stealLife(LivingAttackEvent event) {
        if (Configs.isEnabled(EnchantmentLifeStealingConfig.class)
            && event.source.getEntity() instanceof EntityPlayer) {
            EntityPlayer player = (EntityPlayer) event.source.getEntity();
            ItemStack itemStack = player.getCurrentEquippedItem();
            int enchantmentListID = EnchantmentHelpers
                .doesEnchantApply(itemStack, EnchantmentLifeStealingConfig._instance.ID);
    ...
    private void breakingEvent(LivingAttackEvent event) {
        if (Configs.isEnabled(EnchantmentBreakingConfig.class)
            ...
            ItemStack itemStack = entity.getEquipmentInSlot(0);
            int enchantmentListID = EnchantmentHelpers
                .doesEnchantApply(itemStack, EnchantmentBreakingConfig._instance.ID);
            if (enchantmentListID > -1) {
    ...
    private void poisonTipEvent(LivingAttackEvent event) {
        if (Configs.isEnabled(EnchantmentPoisonTipConfig.class)
            ...
            ItemStack itemStack = entity.getEquipmentInSlot(0);
            int enchantmentListID = EnchantmentHelpers
                .doesEnchantApply(itemStack, EnchantmentPoisonTipConfig._instance.ID);
            if (enchantmentListID > -1) {
                int level = EnchantmentHelpers.getEnchantmentLevel(itemStack, enchantmentListID);
