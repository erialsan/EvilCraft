...
public class LivingDeathEventHook {
    ...
    private void bloodObtainEvent(LivingDeathEvent event) {
        ...
        if (e != null && e instanceof EntityPlayerMP
            ...
            && Configs.isEnabled(BloodExtractorConfig.class)) {
            float boost = 1.0F;
            EntityPlayerMP player = (EntityPlayerMP) e;
            if (Configs.isEnabled(VeinSwordConfig.class) && player.getHeldItem() != null
                && player.getHeldItem()
                    .getItem() == VeinSword.getInstance()) {
            ...
            BloodExtractor.getInstance()
                .fillForAllBloodExtractors(player, minimumMB, maximumMB);
        }
    }

    private void bloodStainedBlockEvent(LivingDeathEvent event) {
        if (event.source == DamageSource.fall && Configs.isEnabled(BloodStainedBlockConfig.class)
            ...
            if (BloodStainedBlock.getInstance()
                .canSetInnerBlock(block, event.entity.worldObj, x, y, z) || block == BloodStainedBlock.getInstance()) {
                if (!event.entity.worldObj.isRemote) {
                    ...
                    EntityBloodSplashFX.spawnParticles(
                        event.entity.worldObj,
                        x,
                        y + 1,
                        z,
                        ((int) event.entityLiving.getMaxHealth()) + random.nextInt(15),
                        5 + random.nextInt(5));
                }
            }
        }
    ...
    private void dropHumanoidFleshEvent(LivingDeathEvent event) {
        if (event.entityLiving instanceof EntityPlayerMP && Configs.isEnabled(WerewolfFleshConfig.class)
