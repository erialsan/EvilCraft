...
public class Werewolf extends EntityMob implements IConfigurable {
    ...
    public static boolean isWerewolfTime(World world) {
        return world.getCurrentMoonPhaseFactor() == 1.0 && !MinecraftHelpers.isDay(world)
    ...
    private static void replaceEntity(EntityLiving old, EntityLiving neww, World world) {
        ...
        // https://github.com/iChun/Morph/blob/master/morph/client/model/ModelMorphAcquisition.java
    ...
    public void onLivingUpdate() {
        if (!worldObj.isRemote
            && (!isWerewolfTime(worldObj) || worldObj.difficultySetting == EnumDifficulty.PEACEFUL)) {
            replaceWithVillager();
        } else {
    ...
    public float getBarkProgressScaled(float scale) {
        if (barkprogress == -1) return 0;
        else return (float) barkprogress / (float) BARKLENGTH * scale;
    }

    @Override
    protected Item getDropItem() {
        if (Configs.isEnabled(WerewolfBoneConfig.class)) return WerewolfBoneConfig._instance.getItemInstance();
        else return super.getDropItem();
    }

    @Override
    protected void dropRareDrop(int chance) {
        if (Configs.isEnabled(WerewolfFurConfig.class)) this.dropItem(WerewolfFurConfig._instance.getItemInstance(), 1);
    }
