...
public class Netherfish extends EntitySilverfish implements IConfigurable {
    ...
    public boolean attackEntityAsMob(Entity entity) {
        ...
        if (this.rand.nextFloat() < FIRE_CHANCE) entity.setFire(this.rand.nextInt(MAX_FIRE_DURATION));
        return super.attackEntityAsMob(entity);
    }

    @Override
    public void onLivingUpdate() {
        // TODO: for some reason, this does not work, although it is called client side, it just doesn't render those
        // damn particles...
    ...
    protected void updateEntityActionState() {
        // This fish does NOT grief, maybe this could be implemented, maybe not, nobody likes griefing mobs, but it is
        // quite evil though...
        if (!this.worldObj.isRemote) {
            if (this.entityToAttack == null && !this.hasPath()) {
                ...
                Block block = this.worldObj.getBlock(
                    i + Facing.offsetsXForSide[i2],
                    j + Facing.offsetsYForSide[i2],
                    k + Facing.offsetsZForSide[i2]);
                int metaData = NetherfishSpawn.getInstance()
                    .getMetadataFromBlock(block);
