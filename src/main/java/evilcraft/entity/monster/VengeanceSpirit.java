...
public class VengeanceSpirit extends EntityNoMob implements IConfigurable {
    ...
    public void writeEntityToNBT(NBTTagCompound tag) {
        ...
        if (getInnerEntity() != null) tag.setString(
            "innerEntity",
            getInnerEntity().getClass()
                .getName());
    ...
    public void readEntityFromNBT(NBTTagCompound tag) {
        ...
        if (name != null) this.dataWatcher.updateObject(WATCHERID_INNER, name);
    ...
    private void spawnSmoke() {
        ...
        if (this.isDead) numParticles *= 10;
        ...
        for (int i = 0; i < numParticles; i++) {
            ...
            EntityDarkSmokeFX particle = new EntityDarkSmokeFX(
                worldObj,
                particleX,
                particleY,
                particleZ,
                ...
                particleMotionY,
                particleMotionZ);
            if (this.isDead) particle.setDeathParticles();
    ...
    private void spawnSwarmParticles() {
        ...
        for (int i = 0; i < numParticles; i++) {
            ...
            EntityDegradeFX particle = new EntityDegradeFX(
                worldObj,
                particleX,
                particleY,
                particleZ,
    ...
    public boolean canEntityBeSeen(Entity entity) {
        if (entity instanceof EntityPlayer) return isEnabledVengeance((EntityPlayer) entity);
        else return super.canEntityBeSeen(entity);
    }
    ...
    public void setEnabledVengeance(EntityPlayer player, boolean enabled) {
        ...
        if (enabled && index == ArrayUtils.INDEX_NOT_FOUND) players = ArrayUtils.add(players, player.getDisplayName());
        else if (!enabled && index != ArrayUtils.INDEX_NOT_FOUND) players = ArrayUtils.remove(players, index);
    ...
    public EntityLivingBase getInnerEntity() {
        ...
        if (innerEntity != null) return innerEntity;
        try {
            Class<EntityLivingBase> clazz = (Class<EntityLivingBase>) Class
                .forName(dataWatcher.getWatchableObjectString(WATCHERID_INNER));
    ...
    public void setInnerEntity(EntityLivingBase innerEntity) {
        if (innerEntity instanceof EntityPlayer) {
            setPlayerId(
                ((EntityPlayer) innerEntity).getGameProfile()
                    .getId()
                    .toString());
            setPlayerName(
                ((EntityPlayer) innerEntity).getGameProfile()
    ...
    public static boolean canSpawnNew(World world, double x, double y, double z) {
        ...
        return WorldHelpers.foldArea(
            world,
            GemStoneTorchConfig.area,
            (int) x,
            (int) y,
            (int) z,
            new WorldHelpers.WorldFoldingFunction<Boolean, Boolean>() {
                ...
                public Boolean apply(@Nullable Boolean input, World world, int x, int y, int z) {
                    return (input == null || input)
                        && world.getBlock(x, y, z) != GemStoneTorchConfig._instance.getBlockInstance();
                }
     ...
     */
    public void onHit(double hitX, double hitY, double hitZ, double impactMotionX, double impactMotionY,
        double impactMotionZ) {
    ...
    @SideOnly(Side.CLIENT)
    private void showBurstParticles(double hitX, double hitY, double hitZ, double impactMotionX, double impactMotionY,
        double impactMotionZ) {
        for (int i = 0; i < worldObj.rand.nextInt(5); i++) {
            ...
            EntityBlurFX blur = new EntityBlurFX(
                worldObj,
                hitX,
                hitY,
                hitZ,
    ...
    public static VengeanceSpirit spawnRandom(World world, int x, int y, int z, int area) {
        ...
        while (canSpawnNew(world, x, y, z) && attempts > 0) {
            int posX = x + MathHelper.getRandomIntegerInRange(world.rand, baseDistance, baseDistance + area)
                * MathHelper.getRandomIntegerInRange(world.rand, -1, 1);
            int posY = y + MathHelper.getRandomIntegerInRange(world.rand, 0, 3)
                * MathHelper.getRandomIntegerInRange(world.rand, -1, 1);
            int posZ = z + MathHelper.getRandomIntegerInRange(world.rand, baseDistance, baseDistance + area)
                * MathHelper.getRandomIntegerInRange(world.rand, -1, 1);
            ...
            if (World.doesBlockHaveSolidTopSurface(world, posX, posY - 1, posZ)) {
                ...
                if (world.checkNoEntityCollision(spirit.boundingBox)
                    && world.getCollidingBoundingBoxes(spirit, spirit.boundingBox)
                        .isEmpty()
