...
public class EntityBroom extends Entity implements IConfigurable {
    ...
    public void setPositionAndRotation2(double x, double y, double z, float yaw, float pitch,
        ...
        if (worldObj.isRemote && Minecraft.getMinecraft().thePlayer == lastMounted) {
            ...
            if (changePosition) setPosition(posX, posY, posZ);
        }
    }
    ...
    public void onUpdate() {
        ...
        if (!worldObj.isRemote && riddenByEntity == null && lastMounted != null) {
            ...
            if (!lastMounted.capabilities.isCreativeMode && Configs.isEnabled(BroomConfig.class)) {
                ...
                else dropItem(Broom.getInstance(), 1);
            }
    ...
    protected void updateMountedServer() {
        ...
        if (rotationPitch > MAX_ANGLE) rotationPitch = MAX_ANGLE;
        else if (rotationPitch < MIN_ANGLE) rotationPitch = MIN_ANGLE;
        ...
        if (lastMounted.moveForward != 0) {
            double playerSpeed = 10 * lastMounted.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
                .getAttributeValue();
        ...
        if (worldObj.isRemote) motionY += getHoverOffset();
