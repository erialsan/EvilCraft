...
public class MobSpawnDegradation extends StochasticDegradationEffect {
    ...
    public static void initInstance(ExtendedConfig<DegradationEffectConfig> eConfig) {
        if (_instance == null) _instance = new MobSpawnDegradation(eConfig);
    ...
    public void runServerSide(IDegradable degradable) {
        ...
        SpawnListEntry spawnlistentry = world.spawnRandomCreature(
            EnumCreatureType.monster,
            spawn.getCoordinates()[0],
            spawn.getCoordinates()[1],
            spawn.getCoordinates()[2]);
        EntityLiving entityliving;

        try {
            entityliving = (EntityLiving) spawnlistentry.entityClass.getConstructor(World.class)
                .newInstance(world);
