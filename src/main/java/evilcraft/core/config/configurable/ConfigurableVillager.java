...
public class ConfigurableVillager implements IConfigurable, IVillageTradeHandler {
    ...
    public void manipulateTradesForVillager(EntityVillager villager, MerchantRecipeList recipeList, Random random) {
        if (villager.getProfession() == eConfig.downCast()
            .getId()) {
            addedRecipes = 0;
            attemptAddRecipe = 0;
     ...
     */
    private boolean tryAddingRecipe(MerchantRecipeList recipeList, Random random, WeightedItemStack inputAWeighted,
        WeightedItemStack inputBWeighted, WeightedItemStack outputWeighted) {
        attemptAddRecipe++;
        ...
        inputA = inputAWeighted.getItemStack()
            .copy();
        inputA.stackSize = getRandomBetween(
            random,
            inputA.stackSize,
            getRandomBetween(random, inputMinStackSize, inputMaxStackSize));
        ...
        if (inputBWeighted != null) {
            inputB = inputAWeighted.getItemStack()
                .copy();
            inputB.stackSize = getRandomBetween(
                random,
                inputB.stackSize,
                getRandomBetween(random, inputMinStackSize, inputMaxStackSize));
        ...
        output = outputWeighted.getItemStack()
            .copy();
        output.stackSize = Math.min(
            64,
            outputWeighted.getItemStack().stackSize * (int) safeDivide(totalInputWeight, outputWeighted.getWeight()));
