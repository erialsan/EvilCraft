...
public abstract class ExtendedConfig<C extends ExtendedConfig<C>>
    ...
    public void save() {
        ...
        try {
            ...
            if (getOverriddenSubInstance() == null && this.getHolderType()
                .hasUniqueInstance()) {
                this.getElement()
                    .getMethod("initInstance", ExtendedConfig.class)
                    .invoke(null, this);
            }
            ...
            if (!this.isDisableable()) {
                throw new EvilCraftConfigException(
                    "Registering " + this.getNamedId()
                        + " caused the issue: "
                        + e.getCause()
                            .getMessage()
    ...
    public IConfigurable getSubInstance() {
        ...
        if (!this.getHolderType()
            .hasUniqueInstance()) throw new EvilCraftConfigException("There exists no unique instance for " + this);
        try {
            return (IConfigurable) this.getElement()
