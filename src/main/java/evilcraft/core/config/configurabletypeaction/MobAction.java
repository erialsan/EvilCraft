...
public class MobAction extends ConfigurableTypeAction<MobConfig> {
    ...
    public void postRun(MobConfig eConfig, Configuration config) {
        ...
        if (MinecraftHelpers.isClientSide()) ClientProxy.ENTITY_RENDERERS.put(clazz, eConfig.getRender());
        if (eConfig.hasSpawnEgg()) {
            ...
            EntityRegistry
                .registerGlobalEntityID(clazz, eConfig.getNamedId(), EntityRegistry.findGlobalUniqueEntityId());
        }
