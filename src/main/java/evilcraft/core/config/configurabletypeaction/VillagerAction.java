...
public class VillagerAction extends ConfigurableTypeAction<VillagerConfig> {
    ...
    public void postRun(VillagerConfig eConfig, Configuration config) {
        ...
        VillagerRegistry.instance()
            .registerVillagerId(eConfig.getId());
        if (MinecraftHelpers.isClientSide()) {
            ResourceLocation villagerSkin = new ResourceLocation(
                Reference.MOD_ID,
                Reference.TEXTURE_PATH_SKINS + eConfig.getNamedId() + ".png");
            VillagerRegistry.instance()
                .registerVillagerSkin(eConfig.getId(), villagerSkin);
        }

        // Add trades
        VillagerRegistry.instance()
            .registerVillageTradeHandler(eConfig.getId(), (ConfigurableVillager) eConfig.getSubInstance());
    }
