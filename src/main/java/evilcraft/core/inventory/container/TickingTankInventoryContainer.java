...
public class TickingTankInventoryContainer<T extends TickingTankInventoryTileEntity<T>>
        ...
        for (TickComponent<T, ITickAction<T>> ticker : tile.getTickers()) {
            if (previousTicks.get(ticker) != ticker.getTick() || force) {
                ...
                icrafting.sendProgressBarUpdate(
                    this,
                    INDEX_OFFSET + tile.getTickers()
                        .size() + index,
                    Math.round(ticker.getRequiredTicks()));
    ...
    public void updateProgressBar(int index, int value) {
        ...
        if (index >= tile.getTickers()
            .size() + INDEX_OFFSET) {
            tile.getTickers()
                .get(
                    index - tile.getTickers()
                        .size() - INDEX_OFFSET)
                .setRequiredTicks(value);
        } else if (index >= INDEX_OFFSET) {
            tile.getTickers()
                .get(index - INDEX_OFFSET)
                .setTick(value);
        }
    }
