...
public class ItemHelpers {
     ...
     */
    public static void tryFillContainerForPlayer(IFluidContainerItem item, ItemStack itemStack, ItemStack toFill,
        FluidStack tickFluid, EntityPlayer player) {
        if (toFill != null && toFill != itemStack
            && toFill.getItem() instanceof IFluidContainerItem
            && !player.isUsingItem()) {
            IFluidContainerItem fluidContainer = (IFluidContainerItem) toFill.getItem();
            FluidStack heldFluid = fluidContainer.getFluid(toFill);
            if (heldFluid == null
                || (heldFluid.isFluidEqual(tickFluid) && heldFluid.amount < fluidContainer.getCapacity(toFill))) {
                int filled = fluidContainer.fill(
                    toFill,
                    new FluidStack(tickFluid.getFluid(), Math.min(tickFluid.amount, MB_FILL_PERTICK)),
                    true);
