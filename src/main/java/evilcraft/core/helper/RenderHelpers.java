...
public class RenderHelpers {
    ...
    static { // Note: the fields from the RenderBlock are INCORRECT! Very good read:
             // http://greyminecraftcoder.blogspot.be/2013/07/rendering-non-standard-blocks.html
     ...
     */
    public static void renderFaceDirection(ForgeDirection renderDirection, RenderBlocks renderer, Block block, double x,
        double y, double z, IIcon blockIconFromSideAndMetadata) {
        try {
            Method method = ObfuscationHelpers.getRenderFaceMethod(renderer, renderDirection);
            // String methodName =
            // ObfuscationHelpers.isObfusicated()?METHODS_RENDERFACE_OBFUSICATED.get(renderDirection):METHODS_RENDERFACE.get(renderDirection);
            // Method method = renderer.getClass().getMethod(methodName, Block.class, double.class, double.class,
            // double.class, IIcon.class);
    ...
    public static void setRenderBlocksUVRotation(RenderBlocks renderer, ForgeDirection side, int rotation) {
         ...
         * try {
         * String fieldName =
         * ObfuscationHelpers.isObfusicated()?FIELDS_UVROTATE_OBFUSICATED.get(side):FIELDS_UVROTATE.get(side);
     ...
     * @param render      The actual fluid renderer.
     */
    public static void renderFluidContext(FluidStack fluid, double x, double y, double z, boolean enableAlpha,
     ...
     */
    public static void renderTileFluidContext(final FluidStack fluid, final double x, final double y, final double z,
        boolean enableAlpha, final TileEntity tile, final IFluidContextRender render) {
        renderFluidContext(fluid, x, y, z, enableAlpha, new IFluidContextRender() {
            ...
            public void renderFluid(FluidStack fluid) {
                ...
                Tessellator.instance
                    .setBrightness(2 * block.getMixedBrightnessForBlock(tile.getWorldObj(), (int) x, (int) y, (int) z));
    ...
    /**
     * Runnable for
     * {@link RenderHelpers#renderFluidContext(FluidStack, double, double, double, boolean, IFluidContextRender)}.
     * 
     * @author rubensworks
     */
