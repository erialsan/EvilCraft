...
public class CubeDetector {
    ...
    protected boolean isValidLocation(World world, ILocation location, IValidationAction action) {
        Block block = world
            .getBlock(location.getCoordinates()[0], location.getCoordinates()[1], location.getCoordinates()[2]);
    ...
    protected boolean isAir(World world, ILocation location) {
        return world
            .isAirBlock(location.getCoordinates()[0], location.getCoordinates()[1], location.getCoordinates()[2]);
    }
    ...
    protected boolean coordinateRecursion(World world, int[][] dimensionEgdes, int[] accumulatedCoordinates,
        ...
        if (accumulatedCoordinates.length == dimensionEgdes.length) { // Leaf of recursion
            ...
            for (int i = dimensionEgdes[dimension][0]; i <= dimensionEgdes[dimension][1]; i++) {
                ...
                int[] newAccumulatedCoordinates = Arrays
                    .copyOf(accumulatedCoordinates, accumulatedCoordinates.length + 1);
     ...
     * @return If the structure is valid for the given edges.
     */
    protected boolean validateDimensionEdges(World world, final int[][] dimensionEgdes, final boolean valid,
        final IValidationAction action) {
    ...
    protected void postValidate(World world, final Size size, int[][] dimensionEgdes, final boolean valid,
        final ILocation originCorner) {
