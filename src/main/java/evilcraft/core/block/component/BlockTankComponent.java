...
public class BlockTankComponent<T extends BlockContainer & IBlockTank> {
     ...
     */
    public boolean onBlockActivatedTank(World world, int x, int y, int z, EntityPlayer player, int side, float motionX,
        ...
        if (tile != null) {
            if (itemStack != null) {
                ...
                if (fluidStack != null) { // Fill the tank.
                    if (canTankBeFilled(tile, fluidStack) && tile.getTank()
                    ...
                    return true;
                } else if (tile.getTank()
                    .getFluidAmount() > 0) { // Drain the tank.
                        ItemStack filledItem = FluidContainerRegistry.fillFluidContainer(
                            tile.getTank()
                                .getFluid(),
                            itemStack);
                ...
                if (itemStack.getItem() instanceof IFluidContainerItem) {
                    ...
                    if (player.isSneaking()) { // Drain from tank into item.
                        if (tile.getTank()
                            .getFluidAmount() > 0) {
                            FluidStack fluidStackTemp = new FluidStack(
                                tile.getTank()
                                    .getFluid()
                                    .getFluid(),
                            ...
                            if (containerItem.fill(itemStack, fluidStackTemp, false) > 0) {
                                tile.getTank()
                                    .drain(containerItem.fill(itemStack, fluidStackTemp, true), true);
                            }
                        }
    ...
    protected static boolean canTankBeFilled(TankInventoryTileEntity tile, FluidStack fluidStack) {
        return tile.getTank()
            .getAcceptedFluid() == null || fluidStack == null
            || tile.getTank()
                .getFluidType() == null
            || tile.getTank()
    ...
    public String getInfoTank(ItemStack itemStack) {
        ...
        if (itemStack.getItem() instanceof IFluidContainerItem) {
            ...
            fluidStack = FluidStack.loadFluidStackFromNBT(
                itemStack.getTagCompound()
                    .getCompoundTag(tank.getTankNBTName()));
