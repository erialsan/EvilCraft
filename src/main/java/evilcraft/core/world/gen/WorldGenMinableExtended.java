...
public class WorldGenMinableExtended extends WorldGenMinable implements IRetroGen {
     ...
     */
    public WorldGenMinableExtended(Block block, int blocksPerVein, int veinsPerChunk, int startY, int endY,
        Block replaceTarget) {
    ...
    protected boolean generate(Chunk chunk, Random rand, int x, int y, int z) {
        ...
        for (int l = 0; l <= this.blocksPerVein; ++l) {
            ...
            double d10 = (double) (MathHelper.sin((float) l * (float) Math.PI / (float) this.blocksPerVein) + 1.0F) * d9
                + 1.0D;
            double d11 = (double) (MathHelper.sin((float) l * (float) Math.PI / (float) this.blocksPerVein) + 1.0F) * d9
                + 1.0D;
                                ...
                                if (cx > 0 && cx < WorldHelpers.CHUNK_SIZE && cz > 0 && cz < WorldHelpers.CHUNK_SIZE) {
                                    ...
                                    if (storage != null) {
                                        ...
                                        if (oldBlock != null && oldBlock != block
                                            ...
                                            && !oldBlock.hasTileEntity(oldMeta)) { // We do not replace TE's, even if
                                                                                   // they are replacable.
