...
public class ImplicitFluidConverter {
    ...
    public FluidStack convert(FluidStack fluid) {
        if (fluid != null && canConvert(fluid.getFluid())) {
            return new FluidStack(target, (int) Math.floor(fluid.amount * converters.get(fluid.getFluid())));
        }
        return null;
    }
     ...
     * 
     * @param target The fluid to convert to.
     * @param fluid  The fluid stack to convert, it must be of the same fluid type of the main target of this converter
     *               instance.
     * @return The converted fluid stack.
     */
    public FluidStack convertReverse(Fluid target, FluidStack fluid) {
        if (canConvert(target) && fluid.getFluid() == this.target) {
            ...
            return new FluidStack(target, (int) Math.floor(ret.amount / converters.get(target)));
        }
        return null;
    }
        ...
        for (String line : config) {
            ...
            if (split.length != 2) {
                throw new IllegalArgumentException(
                    "Invalid line '" + line + "' found for " + "a fluid converter config.");
            }
            Fluid fluid = FluidRegistry.getFluid(split[0]);
            if (fluid == null) {
                EvilCraft.log(
                    "Could not find a fluid by name '" + split[0] + "' for " + "a fluid converter config.",
                    Level.ERROR);
            }
            double ratio = 1.0D;
            try {
                ...
                EvilCraft
                    .log("Invalid ratio '" + split[1] + "' in " + "a fluid converter config, using 1.0.", Level.ERROR);
            }
