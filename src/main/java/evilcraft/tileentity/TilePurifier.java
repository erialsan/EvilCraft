...
public class TilePurifier extends TankInventoryTileEntity {
    ...
    public TilePurifier() {
        super(
            ...
            1,
            FluidContainerRegistry.BUCKET_VOLUME * MAX_BUCKETS,
            PurifierConfig._instance.getNamedId() + "tank",
            FLUID);

        List<Integer> slots = new LinkedList<Integer>();
        slots.add(SLOT_ADDITIONAL);
        slots.add(SLOT_PURIFY);
        for (ForgeDirection direction : DirectionHelpers.DIRECTIONS) addSlotsToSide(direction, slots);
    ...
    private void updateAdditionalItem() {
        ...
        for (baseNextRotation = this.additionalRotation - this.additionalRotation2; baseNextRotation
    ...
    public void showEnchantingEffect() {
        if (worldObj.rand.nextInt(10) == 0) {
            for (int i = 0; i < 1; i++) {
                ...
                FMLClientHandler.instance()
                    .getClient().effectRenderer.addEffect(
                        new EntityEnchantmentTableParticleFX(
                            worldObj,
                            particleX,
                            particleY,
                            particleZ,
