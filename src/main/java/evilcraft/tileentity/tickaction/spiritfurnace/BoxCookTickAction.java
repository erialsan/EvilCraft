...
public class BoxCookTickAction implements ITickAction<TileSpiritFurnace> {
    ...
    public static final Map<Class<? extends EntityLivingBase>, List<WeightedItemStack>> MOBDROP_OVERRIDES = Maps
        .newHashMap();
    ...
    static {
        ...
        overridePlayerDrop(
            "777e7aa3-9373-4511-8d75-f99d23ebe252",
            new ItemStack(Items.dye, 1, 3).setStackDisplayName("Lekkere Stront")); // Davivs69
        ...
        overridePlayerDrop(
            "b2faeaab-fc87-4f91-98d3-836024f268ae",
            new ItemStack(Blocks.furnace).setStackDisplayName("Fuurnas")); // _KillaH229_
        ...
        overridePlayerDrop(
            ...
            new ItemStack(Items.spawn_egg, 1, 91).setStackDisplayName("jeb_")); // jeb_
        overridePlayerDrop(
            "61699b2e-d327-4a01-9f1e-0ea8c3f06bc6",
            new ItemStack(Items.spawn_egg, 1, 91).setStackDisplayName("Dinnerbone")); // Dinnerbone
        overridePlayerDrop(
            "bbb87dbe-690f-4205-bdc5-72ffb8ebc29d",
            new ItemStack(Blocks.cobblestone, 45).setStackDisplayName("direwolf20")); // direwolf20
        overridePlayerDrop("0b7509f0-2458-4160-9ce1-2772b9a45ac2", new ItemStack(Items.porkchop)); // iChun
    }
    public static final ItemStack[] PLAYERDROP_RANDOM = new ItemStack[] { new ItemStack(Items.book),
        new ItemStack(Items.blaze_powder), new ItemStack(Items.quartz), new ItemStack(Items.chainmail_chestplate),
        new ItemStack(Items.fermented_spider_eye), new ItemStack(Items.glass_bottle), new ItemStack(Items.item_frame),
        new ItemStack(Items.minecart), new ItemStack(Items.shears), new ItemStack(Items.reeds),
        new ItemStack(Items.pumpkin_pie), new ItemStack(Items.magma_cream), new ItemStack(Items.saddle),
        new ItemStack(Items.speckled_melon), new ItemStack(Items.iron_hoe), new ItemStack(Items.carrot_on_a_stick),
        new ItemStack(Items.redstone), new ItemStack(Blocks.coal_block), new ItemStack(Blocks.lapis_block),
        new ItemStack(Blocks.soul_sand), new ItemStack(Blocks.gravel), new ItemStack(Blocks.hopper), };
    ...
    public static void overridePlayerDrop(String playerId, ItemStack drop) {
        PLAYERDROP_OVERRIDES.put(
            UUID.fromString(playerId),
            WeightedItemStack.createWeightedList(Sets.newHashSet(new WeightedItemStack(drop, 1))));
    }

    @Override
    public boolean canTick(TileSpiritFurnace tile, ItemStack itemStack, int slot, int tick) {
        if (!tile.isForceHalt() && !tile.isCaughtError()
            && tile.canWork()
            && tile.getTank()
                .getFluidAmount() >= getRequiredMb(tile, 0)
            && getCookStack(tile) != null
            && tile.canConsume(getCookStack(tile))) {
            for (int slotId : tile.getProduceSlots()) {
                ItemStack production = tile.getInventory()
    ...
    protected ItemStack getPlayerSkull(String playerName) {
        ...
        itemStack.getTagCompound()
            .setString("SkullOwner", playerName);
        return itemStack;
    }
    ...
    protected void doNextDrop(TileSpiritFurnace tile) {
        ...
        if (entity != null) {
            ...
            if (SpiritFurnaceConfig.mobDeathSounds) {
                EvilCraft.proxy.sendSoundMinecraft(
                    tile.xCoord + 0.5D,
                    tile.yCoord + 0.5D,
                    tile.zCoord + 0.5D,
                    deathSound,
            ...
            if (tile.isPlayer()) {
                ...
                WeightedItemStack weightedItemStack = WeightedItemStack
                    .getRandomWeightedItemStack(possibleDrops, world.rand);
    ...
    protected int getRequiredMb(TileSpiritFurnace tile, int tick) {
        MutableDouble drain = new MutableDouble(
            tile.isPlayer() ? SpiritFurnaceConfig.playerMBPerTick : SpiritFurnaceConfig.mBPerTick);
        Upgrades.sendEvent(
            tile,
            new UpgradeSensitiveEvent<MutableDouble>(drain, TileSpiritFurnace.UPGRADEEVENT_BLOODUSAGE));
    ...
    public void onTick(TileSpiritFurnace tile, ItemStack itemStack, int slot, int tick) {
        ...
        tile.getTank()
            .drain(getRequiredMb(tile, tick), true);
    ...
    public float getRequiredTicks(TileSpiritFurnace tile, int slot, int tick) {
        ...
        if (entity == null) {
            ...
            requiredTicksBase = (int) ((entity.getHealth() + entity.getTotalArmorValue())
                * SpiritFurnaceConfig.requiredTicksPerHp);
        }
        MutableDouble duration = new MutableDouble(requiredTicksBase);
        Upgrades
            .sendEvent(tile, new UpgradeSensitiveEvent<MutableDouble>(duration, TileSpiritFurnace.UPGRADEEVENT_SPEED));
