...
public class AccumulateItemTickAction implements ITickAction<TileSanguinaryEnvironmentalAccumulator> {
    ...
    public boolean canTick(TileSanguinaryEnvironmentalAccumulator tile, ItemStack itemStack, int slot, int tick) {
        ...
        if (tile.getStackInSlot(TileSanguinaryEnvironmentalAccumulator.SLOT_ACCUMULATE) != null
            && tile.canConsume(tile.getStackInSlot(TileSanguinaryEnvironmentalAccumulator.SLOT_ACCUMULATE))) {
            ItemStack production = tile.getInventory()
                .getStackInSlot(tile.getProduceSlot());
            ItemStack willProduce = willProduceItem(tile);
            if (production == null) {
                precondition = true;
            } else if (willProduce != null && production.getItem() == willProduceItem(tile).getItem()
                && production.getItemDamage() == willProduceItem(tile).getItemDamage()) {
                    if (production.stackSize + willProduce.stackSize <= production.getMaxStackSize())
                        precondition = true;
                }
        }
        return precondition && tile.canWork()
            && tile.getVirtualTank()
                .getFluidAmount() >= getRequiredFluidAmount(tile, getRecipe(tile));
    }
                ...
                if (addToProduceSlot(
                    ...
                    recipe.getProperties()
                        ...
                        .getResult(
                            ...
                            tile.zCoord,
                            recipe.getOutput()
                                .getConditionalItemStack(itemStack)))) {
                    tile.getInventory()
                        .decrStackSize(tile.getConsumeSlot(), 1);
                    tile.getVirtualTank()
                        .drain(getRequiredFluidAmount(tile, recipe), true);
                }
            }
        }
    }
    ...
    public final float getRequiredTicks(TileSanguinaryEnvironmentalAccumulator tile, int slot, int tick) {
        ...
        Upgrades.sendEvent(
            tile,
            new UpgradeSensitiveEvent<MutableInt>(duration, TileSanguinaryEnvironmentalAccumulator.UPGRADEEVENT_SPEED));
    ...
    protected int getRequiredFluidAmount(TileSanguinaryEnvironmentalAccumulator tile,
        ...
        Upgrades.sendEvent(
            tile,
            new UpgradeSensitiveEvent<MutableInt>(
                amount,
                TileSanguinaryEnvironmentalAccumulator.UPGRADEEVENT_BLOODUSAGE));
        return Math.max(1, amount.getValue());
    }

    private IRecipe<EnvironmentalAccumulatorRecipeComponent, EnvironmentalAccumulatorRecipeComponent, EnvironmentalAccumulatorRecipeProperties> getRecipe(
        TileSanguinaryEnvironmentalAccumulator tile) {
    ...
    protected ItemStack willProduceItem(TileSanguinaryEnvironmentalAccumulator tile) {
        return getRecipe(tile).getOutput()
            .getConditionalItemStack(tile.getStackInSlot(TileSanguinaryEnvironmentalAccumulator.SLOT_ACCUMULATE));
    }
