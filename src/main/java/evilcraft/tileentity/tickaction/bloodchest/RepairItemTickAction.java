...
public class RepairItemTickAction implements ITickAction<TileBloodChest> {
    ...
    private void drainTank(TileBloodChest tile, float usageMultiplier) {
        tile.getTank()
            .drain((int) Math.ceil((float) BloodChestConfig.mBPerDamage * usageMultiplier), true);
    }
            ...
            if (!tile.getTank()
                ...
                if (actionID > -1) {
                    float simulateMultiplier = actions
                        .repair(itemStack, tile.getWorldObj().rand, actionID, false, false);
                    if (tile.getTank()
                        .getFluidAmount() >= BloodChestConfig.mBPerDamage * simulateMultiplier) {
