...
public abstract class BloodInfuserTickAction implements ITickAction<TileBloodInfuser> {
    ...
    public boolean canTick(TileBloodInfuser tile, ItemStack itemStack, int slot, int tick) {
        ...
        if (!tile.getTank()
            .isEmpty() && getInfuseStack(tile) != null && tile.canConsume(getInfuseStack(tile))) {
            ...
            if (production == null) {
                ...
                && production.getItemDamage() == willProduceItem(tile).getItemDamage()) {
                    if (production.stackSize + willProduce.stackSize <= production.getMaxStackSize()) return true;
                }
        }
        return false;
    }
