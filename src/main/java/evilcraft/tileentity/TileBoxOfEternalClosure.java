...
public class TileBoxOfEternalClosure extends EvilCraftTileEntity {
    ...
    public void updateTileEntity() {
        ...
        if (spirit == null && !worldObj.isRemote) {
            if (target != null || (WorldHelpers.efficientTick(getWorldObj(), TICK_MODULUS, xCoord, yCoord, zCoord)
                && findNextEntity())) {
                pullEntity();
            }
        }
    ...
    private boolean findNextEntity() {
        AxisAlignedBB box = AxisAlignedBB.getBoundingBox(xCoord, yCoord, zCoord, xCoord, yCoord, zCoord)
            .expand(TARGET_RADIUS, TARGET_RADIUS, TARGET_RADIUS);
    ...
    private void pullEntity() {
        ...
        if (target != null) {
            ...
            if (target.isDead || !target.isFrozen()) {
                ...
                if (target.boundingBox.expand(ABSORB_RADIUS, ABSORB_RADIUS, ABSORB_RADIUS)
                    .intersectsWith(getBlock().getCollisionBoundingBoxFromPool(worldObj, xCoord, yCoord, zCoord))) {
                    closing = true;
                    close(true);
    ...
    public void releaseSpirit() {
        ...
        spirit.setPosition(xCoord + rand.nextDouble(), yCoord + rand.nextDouble(), zCoord + rand.nextDouble());
        spirit.setFrozenDuration(0);
        spirit.setGlobalVengeance(true);
        spirit.setRemainingLife(
            MathHelper.getRandomIntegerInRange(
                worldObj.rand,
                VengeanceSpirit.REMAININGLIFE_MIN,
                VengeanceSpirit.REMAININGLIFE_MAX));
    ...
    public static EntityLivingBase getEntityFromNBT(World world, NBTTagCompound tag) {
        if (tag != null && tag.hasKey(EntityHelpers.NBTTAG_ID)) {
            return (EntityLivingBase) EntityList.createEntityFromNBT(tag, world);
        }
        return null;
    }
