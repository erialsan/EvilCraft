...
public class TileEnvironmentalAccumulator extends EvilCraftBeaconTileEntity
    implements IBossDisplayData, IDegradable, IInventory {
    ...
    private static final double WEATHER_CONTAINER_SPAWN_HEIGHT = EnvironmentalAccumulatorConfig.defaultProcessItemTickCount
    ...
    private static final ILocation[] waterOffsets = new ILocation[] { new Location(-2, -1, -2), new Location(-2, -1, 2),
        new Location(2, -1, -2), new Location(2, -1, 2), };
    ...
    private Vector4f getInnerColorByState(int state) {
        ...
        else return new Vector4f(0, 0, 0, 0.13f);

    }

    @SideOnly(Side.CLIENT)
    private Vector4f getOuterColorByState(int state) {
        if (state == EnvironmentalAccumulator.STATE_COOLING_DOWN) return new Vector4f(0, 0, 0, 0.13f);
        else return new Vector4f(0.30078125F, 0.1875F, 0.08203125F, 0.13f);
    }
    ...
    public int getMaxCooldownTick() {
        ...
        if (result == null) return EnvironmentalAccumulatorConfig.defaultTickCooldown;
        else return result.getCooldownTime();
    }
    ...
    private int getItemMoveDuration() {
        if (recipe == null) return EnvironmentalAccumulatorConfig.defaultProcessItemTickCount;
        else return recipe.getProperties()
            .getDuration();
    }

    private float getItemMoveSpeed() {
        if (recipe == null) return (float) EnvironmentalAccumulatorConfig.defaultProcessItemSpeed;
        else return (float) recipe.getProperties()
            .getProcessingSpeed();
    }
    ...
    @SideOnly(Side.CLIENT)
    public static void showAccumulatingParticles(World world, double centerX, double centerY, double centerZ,
        double spread) {
    ...
    private void updateEnvironmentalAccumulatorIdle() {
        ...
        List entityItems = worldObj.getEntitiesWithinAABB(
            EntityItem.class,
            AxisAlignedBB.getBoundingBox(
                this.xCoord,
                this.yCoord + WEATHER_CONTAINER_MIN_DROP_HEIGHT,
                this.zCoord,
                this.xCoord + 1.0,
                this.yCoord + WEATHER_CONTAINER_MAX_DROP_HEIGHT,
                this.zCoord + 1.0));
        ...
        for (IRecipe<EnvironmentalAccumulatorRecipeComponent, EnvironmentalAccumulatorRecipeComponent, EnvironmentalAccumulatorRecipeProperties> recipe : EnvironmentalAccumulator
            ...
            for (Object obj : entityItems) {
                ...
                if (recipeStack.getItem() == stack.getItem() && recipeStack.getItemDamage() == stack.getItemDamage()
    ...
    private void dropItemStack() {
        if (!worldObj.isRemote) {
            ...
            EntityItem entity = new EntityItem(
                worldObj,
                this.xCoord,
                this.yCoord + WEATHER_CONTAINER_SPAWN_HEIGHT,
                this.zCoord);
                ...
                entity.setEntityItemStack(
                    recipe.getProperties()
                        ...
                        .getResult(
                            ...
                            zCoord,
                            recipe.getOutput()
                                .getConditionalItemStack(this.getStackInSlot(0))));

                // Change the weather to the resulting weather
                WeatherType weatherSource = recipe.getInput()
                    .getWeatherType();
                if (weatherSource != null) weatherSource.deactivate(worldObj);

                WeatherType weatherResult = recipe.getOutput()
                    .getWeatherType();
                if (weatherResult != null) weatherResult.activate(worldObj);
            }
    ...
    private void activateProcessingItemState() {
        ...
        if (recipe == null) tick = EnvironmentalAccumulatorConfig.defaultProcessItemTickCount;
        else tick = recipe.getProperties()
            .getDuration();
    ...
    public void onUpdateReceived() {
        ...
        if (worldObj.isRemote && state == EnvironmentalAccumulator.STATE_FINISHED_PROCESSING_ITEM) {
            ...
            IEAProcessingFinishedEffect effect = (recipe == null) ? null
                : recipe.getProperties()
                    .getFinishedProcessingEffect();

            if (effect == null) // fall back to default case
                this.worldObj.playAuxSFX(
                    2002,
                    (int) Math.round(xCoord),
                    (int) Math.round(yCoord + WEATHER_CONTAINER_SPAWN_HEIGHT),
                    (int) Math.round(zCoord),
                    16428);
            else effect.executeEffect(this, recipe);
        }
    ...
    public void readFromNBT(NBTTagCompound compound) {
        ...
        if (recipeId != null) recipe = EnvironmentalAccumulator.getInstance()
            .getRecipeRegistry()
            .findRecipeByNamedId(recipeId);
    ...
    public void writeToNBT(NBTTagCompound compound) {
        ...
        if (recipeId != null) compound.setString("recipe", recipeId);
    ...
    public float getMaxHealth() {
        if (state == EnvironmentalAccumulator.STATE_PROCESSING_ITEM) return getItemMoveDuration();

        if (state == EnvironmentalAccumulator.STATE_FINISHED_PROCESSING_ITEM) return 0;
    ...
    public float getHealth() {
        if (state == EnvironmentalAccumulator.STATE_PROCESSING_ITEM) return tick;

        if (state == EnvironmentalAccumulator.STATE_COOLING_DOWN) return getMaxCooldownTick() - tick;
    ...
    public IChatComponent func_145748_c_() {
        String message = L10NHelpers.localize(
            "chat.bossDisplay.charge",
    ...
    public ItemStack getStackInSlot(int i) {
        if (i >= getSizeInventory() || i < 0) return null;
