...
public abstract class TileWorking<T extends TankInventoryTileEntity, O> extends WorkingTileEntity<T, O> {
    ...
    public Upgrades.Upgrade getUpgradeType(ItemStack itemStack) {
        if (itemStack.getItem() == UPGRADE_ITEM) {
            Upgrades.Upgrade upgrade = Promise.getInstance()
                .getUpgrade(itemStack);
    ...
    public int getTier() {
        if (currentTier == -1) {
            for (int i = getBasicInventorySize(); i < getBasicInventorySize() + INVENTORY_SIZE_UPGRADES; i++) {
                ...
                if (Promise.getInstance()
                    .isTierUpgrade(itemStack)) {
                    currentTier = Promise.getInstance()
                        .getUpgrade(itemStack)
                        .getTier();
                }
            }
    ...
    protected boolean areUpgradeSlotsValidForTier(int tier) {
        for (int i = getBasicInventorySize() + tier + 1; i < getBasicInventorySize() + INVENTORY_SIZE_UPGRADES; i++) {
            if (getStackInSlot(i) != null && !Promise.getInstance()
                .isTierUpgrade(getStackInSlot(i))) return false;
        }
        return true;
    }
    ...
    public boolean canInsertItem(int slot, ItemStack itemStack) {
        ...
        if (Promise.getInstance()
            .isTierUpgrade(itemStack)
            && Promise.getInstance()
                .isTierUpgrade(getStackInSlot(slot))) {
            // Condition already checked in canExtractItem.
            return true;
        } else if (Promise.getInstance()
            .isTierUpgrade(getStackInSlot(slot))) {
                ...
                .isTierUpgrade(itemStack)) {
                    // Add a tier upgrade
                    return getTier() == 0 && getRequiredTierForSlot(slot) <= Promise.getInstance()
                        .getUpgrade(itemStack)
                        .getTier();
                } else {
                    // Non-tier upgrade
    ...
    public boolean canExtractItem(int slot, ItemStack itemStack, ItemStack replaceItem) {
        ...
        if (Promise.getInstance()
            .isTierUpgrade(itemStack)
            && Promise.getInstance()
                .isTierUpgrade(replaceItem)) {
            // Swap tier upgrades
            int newTier = Promise.getInstance()
                .getUpgrade(replaceItem)
                .getTier();
            int currentTier = Promise.getInstance()
                .getUpgrade(itemStack)
                .getTier();
