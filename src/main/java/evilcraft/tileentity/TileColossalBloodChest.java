...
public class TileColossalBloodChest extends TileWorking<TileColossalBloodChest, MutableFloat> {
     ...
     */
    public static final int SLOTS_CHEST = ContainerColossalBloodChest.CHEST_INVENTORY_ROWS
        * ContainerColossalBloodChest.CHEST_INVENTORY_COLUMNS;
    /**
    ...
    static {
        EMPTY_IN_TANK_TICK_ACTIONS
            .put(IFluidContainerItem.class, new EmptyFluidContainerInTankTickAction<TileColossalBloodChest>());
    ...
    public static CubeDetector detector = new HollowCubeDetector(
        new AllowedBlock[] { new AllowedBlock(ReinforcedUndeadPlank.getInstance()),
    ...
    public TileColossalBloodChest() {
        super(
            SLOTS,
            ColossalBloodChest.getInstance()
                .getLocalizedName(),
            LIQUID_PER_SLOT,
        ...
        for (int i = 0; i < SLOTS_CHEST; i++) {
            addTicker(
                new TickComponent<TileColossalBloodChest, ITickAction<TileColossalBloodChest>>(
                    this,
                    REPAIR_TICK_ACTIONS,
                    i));
        }
    ...
    public boolean isItemValidForSlot(int slot, ItemStack itemStack) {
        ...
        else if (slot <= SLOTS_CHEST && slot >= 0) return SlotRepairable.checkIsItemValid(itemStack);
        return false;
    }
    ...
    public void updateTileEntity() {
        ...
        if (worldObj != null && !this.worldObj.isRemote
            && this.worldObj.getWorldTime() % ColossalBloodChestConfig.ticksPerDamage == 0) {
        ...
        if (worldObj != null && !this.worldObj.isRemote
            ...
            && WorldHelpers.efficientTick(
                ...
                this.xCoord,
                this.yCoord,
                this.zCoord)/* (this.ticksSinceSync + this.xCoord + this.yCoord + this.zCoord) % 200 == 0 */) {
            this.playersUsing = 0;
            float range = 5.0F;
            @SuppressWarnings("unchecked")
