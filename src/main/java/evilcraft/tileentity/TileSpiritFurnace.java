...
public class TileSpiritFurnace extends TileWorking<TileSpiritFurnace, MutableDouble> implements IItemDropListener {
    ...
    public static CubeDetector detector = new HollowCubeDetector(
        new AllowedBlock[] { new AllowedBlock(DarkBloodBrick.getInstance()),
    ...
    static {
        EMPTY_IN_TANK_TICK_ACTIONS
            .put(IFluidContainerItem.class, new EmptyFluidContainerInTankTickAction<TileSpiritFurnace>());
    ...
    public TileSpiritFurnace() {
        super(
            SLOTS,
            SpiritFurnace.getInstance()
                .getLocalizedName(),
            LIQUID_PER_SLOT,
            TileSpiritFurnace.TANKNAME,
            ACCEPTED_FLUID);
        cookTicker = addTicker(
            new TickComponent<TileSpiritFurnace, ITickAction<TileSpiritFurnace>>(
                this,
                BOX_COOK_TICK_ACTIONS,
                SLOT_BOX));
        addTicker(
            new TickComponent<TileSpiritFurnace, ITickAction<TileSpiritFurnace>>(
                this,
    ...
    public EntityLiving getEntity() {
        ...
        if (boxStack != null && boxStack.getItem() == getAllowedCookItem()) {
            String id = BoxOfEternalClosure.getInstance()
                .getSpiritName(boxStack);
            if (id != null) {
                ...
                if (boxEntityCache != null && id.equals(EntityList.getEntityString(boxEntityCache))) {
                    ...
                    @SuppressWarnings("unchecked")
                    Class<? extends EntityLivingBase> entityClass = (Class<? extends EntityLivingBase>) EntityList.stringToClassMapping
                        .get(id);
                    if (entityClass != null) {
    ...
    public static void detectStructure(World world, ILocation location, Size size, boolean valid) {
        ...
        if (change) {
            ...
            PacketHandler.sendToAllAround(
                new DetectionListenerPacket(location, valid),
