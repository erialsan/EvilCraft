...
public class DarkTempleGenerator implements IWorldGenerator {
    ...
    private static DarkTempleData loadData(World world) {
        DarkTempleData data = (DarkTempleData) world.perWorldStorage
            .loadData(DarkTempleData.class, DARK_TEMPLE_MAP_NAME);
        if (data == null) {
    ...
    public static @Nullable ILocation getClosestForCoords(World world, int x, int z) {
        ...
        return new Location(
            closest.getLeft() * WorldHelpers.CHUNK_SIZE,
            0,
            closest.getRight() * WorldHelpers.CHUNK_SIZE);
    }

    @Override
    public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator,
        IChunkProvider chunkProvider) {
        if (canGenerate(world) && Configs.isEnabled(EnvironmentalAccumulatorConfig.class)
            && appliesAt(world, chunkX, chunkZ)) {
            int x = chunkX * 16 + random.nextInt(16);
            int y = 0;
            int z = chunkZ * 16 + random.nextInt(16);

            // Generate the dark temple if possible (height checks are performed inside generate)
            if (!DarkTempleStructure.getInstance()
