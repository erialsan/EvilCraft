...
public class EvilDungeonStructure extends WorldGenDungeons {
    ...
    public boolean generate(World world, Random random, int x, int y, int z) {
        ...
        int openingCounter = 0; // Counts the amount of 'holes' that would be available when the dungeon should be
                                // placed
            ...
            for (int yr = y - 1; yr <= y + height + 1; ++yr) {
                for (int zr = z - radiusZ - 1; zr <= z + radiusZ + 1; ++zr) {
                    ...
                    if (!world.getChunkProvider()
                        .chunkExists(xr / 16, yr / 16)) {
                        return false;
                    }

                    Material material = world.getBlock(xr, yr, zr)
                        .getMaterial();
                    if (yr == y - 1 && !material.isSolid()) return false;
                    if (yr == y + height + 1 && !material.isSolid()) return false;
                    if ((xr == x - radiusX - 1 || xr == x + radiusX + 1
                        || zr == z - radiusZ - 1
                        || zr == z + radiusZ + 1) && yr == y
                        && world.isAirBlock(xr, yr, zr)
                        && world.isAirBlock(xr, yr + 1, zr)) ++openingCounter;
                }
            }
        }
        ...
        if (openingCounter >= 1 && openingCounter <= 15) {
            for (int xr = x - radiusX - 1; xr <= x + radiusX + 1; ++xr) {
                for (int yr = y + height; yr >= y - 1; --yr) {
                    for (int zr = z - radiusZ - 1; zr <= z + radiusZ + 1; ++zr) {
                        if (xr != x - radiusX - 1 && yr != y - 1
                            && zr != z - radiusZ - 1
                            && xr != x + radiusX + 1
                            && yr != y + height + 1
                            && zr != z + radiusZ + 1) {
                            world.setBlockToAir(xr, yr, zr);
                        } else if (yr >= 0 && !world.getBlock(xr, yr - 1, zr)
                            .getMaterial()
                            .isSolid()) {
                                world.setBlockToAir(xr, yr, zr);
                            } else if (world.getBlock(xr, yr, zr)
                                .getMaterial()
                                .isSolid()) {
                                    if (yr == y - 1 && random.nextInt(4) != 0) {
                                        if (Configs.isEnabled(BloodyCobblestoneConfig.class)) {
                                            world.setBlock(
                                                ...
                                                yr,
                                                zr,
                                                BloodyCobblestoneConfig._instance.getBlockInstance(),
                                                0,
                                                2);
                                        }
                                    } else {
                    ...
                    if (wallCounter == 1) {
                        ...
                        if (tileentitychest != null) {
                            ...
                            WeightedRandomChestContent.generateChestContents(
                                random,
                                info.getItems(random),
                                tileentitychest,
                                info.getCount(random) * 2);
                        }
                ...
                for (int zs = z - 1; zs <= z + 1; zs += 2) {
                    ...
                    if (tileentitymobspawner != null) {
                        ...
                        tileentitymobspawner.func_145881_a()
                            .setEntityName(this.pickMobSpawner(random));
                    } else {
