...
public class NEIBloodInfuserManager extends TemplateRecipeHandler {
    ...
    protected class CachedBloodInfuserRecipe extends CachedRecipe {
        ...
        public CachedBloodInfuserRecipe(
        ...
        public CachedBloodInfuserRecipe(List<ItemStack> inputStacks, ItemStack outputStack, FluidStack fluidStack,
    ...
    public static class FluidPair {
        ...
        public FluidPair(ItemStack itemStack, FluidStack fluidStack) {
            this.stack = new PositionedStack(
                itemStack,
                FLUID_CONTAINER_X + xOffset,
                FLUID_CONTAINER_Y + yOffset,
                false);
            this.fluidStack = fluidStack;
        }
    ...
    public TemplateRecipeHandler newInstance() {
        if (afluids == null || afluids.isEmpty()) findFluids();
    ...
    private static void findFluids() {
        ...
        for (FluidContainerRegistry.FluidContainerData fluidContainerData : FluidContainerRegistry
            .getRegisteredFluidContainerData()) {
            if (BloodFluidConverter.getInstance()
                .canConvert(fluidContainerData.fluid.getFluid())) {
        ...
        for (ItemStack item : ItemList.items) {
            if (item.getItem() instanceof IFluidContainerItem) {
                ...
                try {
                    ...
                    if (fluidStack != null && BloodFluidConverter.getInstance()
                        .canConvert(fluidStack.getFluid())) {
    ...
    private List<CachedBloodInfuserRecipe> getRecipes() {
        ...
        for (IRecipe<ItemFluidStackAndTierRecipeComponent, ItemStackRecipeComponent, DurationXpRecipeProperties> recipe : BloodInfuser
            .getInstance()
            .getRecipeRegistry()
            .allRecipes()) recipes.add(new CachedBloodInfuserRecipe(recipe));
    ...
    public void loadCraftingRecipes(ItemStack result) {
        List<IRecipe<ItemFluidStackAndTierRecipeComponent, ItemStackRecipeComponent, DurationXpRecipeProperties>> recipes = BloodInfuser
            .getInstance()
            .getRecipeRegistry()
            .findRecipesByOutput(new ItemStackRecipeComponent(result));
