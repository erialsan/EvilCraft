...
public class VersionStats {
                    ...
                    if (GeneralConfig.versionChecker && needsUpdate(versionStats)) {
                        sendMessage(
                            ...
                            L10NHelpers.localize(
                                "general.versionUpdate",
                                versionStats.mod_version,
                                Reference.MOD_NAME,
                                Reference.MOD_VERSION,
                                versionStats.update_link));
                    }
                }
    ...
    private static boolean needsUpdate(VersionStats versionStats) {
        if (versionStats != null) {
            if (!Reference.MOD_VERSION.equals(versionStats.mod_version)) return true;
        }
        return false;
    }
    ...
    private static VersionStats fetchVersionStats() {
        ...
        try {
            Gson gson = new Gson();
            String location = Reference.URL_VERSIONSTATS + "?mc_version="
                + Reference.MOD_MC_VERSION
                + "&mod_version="
                + Reference.MOD_VERSION;
