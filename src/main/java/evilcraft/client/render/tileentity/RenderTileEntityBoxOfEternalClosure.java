...
public class RenderTileEntityBoxOfEternalClosure extends RenderTileEntityModel {
    ...
    private static final ResourceLocation beamTexture = new ResourceLocation(
        Reference.MOD_ID,
        Reference.TEXTURE_PATH_ENTITIES + "beam.png");
    ...
    protected void renderTileEntityAt(EvilCraftTileEntity tile, double x, double y, double z, float partialTick) {
        ...
        if (target != null) {
            ...
            float rotateX = -(target.width / 2)
                - (float) (box.xCoord - target.posX - (target.prevPosX - target.posX) * (double) (1.0F - partialTick));
            float rotateY = (target.height / 2) - (float) ((double) yOffset + box.yCoord
                - target.posY
                - (target.prevPosY - target.posY) * (double) (1.0F - partialTick));
            float rotateZ = -(target.width / 2)
                - (float) (box.zCoord - target.posZ - (target.prevPosZ - target.posZ) * (double) (1.0F - partialTick));
            ...
            GL11.glRotatef(
                (float) (-Math.atan2((double) rotateZ, (double) rotateX)) * 180.0F / (float) Math.PI - 90.0F,
                0.0F,
                1.0F,
                0.0F);
            GL11.glRotatef(
                (float) (-Math.atan2((double) distance, (double) rotateY)) * 180.0F / (float) Math.PI - 90.0F,
                1.0F,
                0.0F,
                0.0F);
            ...
            float v1 = MathHelper.sqrt_float(rotateX * rotateX + rotateY * rotateY + rotateZ * rotateZ) / 32.0F
            ...
            for (int i = 0; i <= amount; ++i) {
                ...
                tessellator
                    .addVertexWithUV((double) (xuv * 0.2F), (double) (yuv * 0.2F), 0.0D, (double) u, (double) v1);
