...
public class EntityPlayerTargettedBlurFX extends EntityBlurFX {
     ...
     */
    public EntityPlayerTargettedBlurFX(World world, float scale, double motionX, double motionY, double motionZ,
        float red, float green, float blue, float ageMultiplier, EntityPlayer player) {
        super(
            world,
            player.posX,
            player.posY,
    ...
    public void onUpdate() {
        ...
        posY = player.posY - 0.5F
            + motionY * f
            + (double) (1.0F - f1)
            + (Minecraft.getMinecraft().thePlayer == player ? 0 : 1);
