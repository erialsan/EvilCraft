...
public class CommandConfigSet extends CommandEvilCraft {
    ...
    public void processCommand(ICommandSender icommandsender, String[] astring) {
        if (astring.length == 0 || astring.length > 1) {
            ...
            if (newValue != null) {
                Configuration configuration = ConfigHandler.getInstance()
                    .getConfig();
                configuration.load();
                config.getCallback()
                    .run(newValue);
                ...
                icommandsender.addChatMessage(
                    new ChatComponentText(
                        L10NHelpers.localize("chat.command.updatedValue", name, newValue.toString())));
            } else {
                icommandsender
                    .addChatMessage(new ChatComponentText(L10NHelpers.localize("chat.command.invalidNewValue")));
            }
        }
    }
