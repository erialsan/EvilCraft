    ...
    public ContainerBloodInfuser(InventoryPlayer inventory, TileBloodInfuser tile) {
        ...
        addSlotToContainer(
            new SlotFluidContainer(
                tile,
                TileBloodInfuser.SLOT_CONTAINER,
                SLOT_CONTAINER_X,
                SLOT_CONTAINER_Y,
                tile.getTank())); // Container emptier
        addSlotToContainer(
            new SlotWorking<TileBloodInfuser>(TileBloodInfuser.SLOT_INFUSE, SLOT_INFUSE_X, SLOT_INFUSE_Y, tile)); // Infuse
                                                                                                                  // slot
        addSlotToContainer(
            new SlotRemoveOnly(tile, TileBloodInfuser.SLOT_INFUSE_RESULT, SLOT_INFUSE_RESULT_X, SLOT_INFUSE_RESULT_Y) {
                ...
                public void onPickupFromSlot(EntityPlayer player, ItemStack itemStack) {
                    ...
                    if (recipe != null) {
                        EntityHelpers.spawnXpAtPlayer(
                            player.worldObj,
                            player,
                            (int) Math.floor(
                                recipe.getProperties()
                                    .getXp() * itemStack.stackSize));
                        FMLCommonHandler.instance()
                            .bus()
                            .post(new BloodInfuserRemoveEvent(player, itemStack));
                    }
