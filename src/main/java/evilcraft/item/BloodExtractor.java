...
public class BloodExtractor extends ConfigurableDamageIndicatedItemFluidContainer {
    ...
    public static void initInstance(ExtendedConfig<ItemConfig> eConfig) {
        if (_instance == null) _instance = new BloodExtractor(eConfig);
    ...
    public boolean onItemUseFirst(ItemStack itemStack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (player.isSneaking()) {
            if (Configs.isEnabled(BloodStainedBlockConfig.class) && block == BloodStainedBlock.getInstance()) {
                ...
                int amount = 0;
                try {
                    amount = ((TileBloodStainedBlock) BloodStainedBlock.getInstance()
                        .getTile(world, x, y, z)).getAmount();
                ...
                BloodStainedBlock.getInstance()
                    .unstainBlock(world, new Location(x, y, z), filled);
