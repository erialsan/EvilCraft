...
public class VengeanceRing extends ConfigurableItem implements IBauble {
    ...
    private static final int[][] RING_POWERS = { { Potion.jump.id, BONUS_POTION_DURATION, 2 },
        { Potion.invisibility.id, BONUS_POTION_DURATION, 1 }, { Potion.moveSpeed.id, BONUS_POTION_DURATION, 1 },
    ...
    public static void initInstance(ExtendedConfig<ItemConfig> eConfig) {
        if (_instance == null) _instance = new VengeanceRing(eConfig);
    ...
    public ItemStack onItemRightClick(ItemStack itemStack, World world, EntityPlayer player) {
        if (player.isSneaking()) {
            if (!world.isRemote) ItemHelpers.toggleActivation(itemStack);
            return itemStack;
        }
    ...
    public static void toggleVengeanceArea(World world, Entity entity, int area, boolean enableVengeance,
        boolean spawnRandom, boolean forceGlobal) {
        if (world.difficultySetting != EnumDifficulty.PEACEFUL) {
            ...
            List<Entity> spirits = world.getEntitiesWithinAABBExcludingEntity(entity, box, new IEntitySelector() {
            ...
            for (Entity found : spirits) {
                VengeanceSpirit spirit = (VengeanceSpirit) found;
            ...
            if (spirits.size() == 0 && enableVengeance) {
                VengeanceSpirit spirit = VengeanceSpirit
                    .spawnRandom(world, (int) Math.round(x), (int) Math.round(y), (int) Math.round(z), area / 4);
                if (spirit != null) {
                    if (forceGlobal) {
