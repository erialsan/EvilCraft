...
public class ExaltedCrafter extends ItemGui implements IItemEmpowerable {
    ...
    public static void initInstance(ExtendedConfig<ItemConfig> eConfig) {
        if (_instance == null) _instance = new ExaltedCrafter(eConfig);
    ...
    private ExaltedCrafter(ExtendedConfig<ItemConfig> eConfig) {
        ...
        if (MinecraftHelpers.isClientSide()) setGUI(GuiExaltedCrafter.class);
    ...
    public void addInformation(ItemStack itemStack, EntityPlayer entityPlayer, List list, boolean par4) {
        ...
        if (isEmpowered(itemStack)) list.add(EnumChatFormatting.RED + "Empowered");
    }
    ...
    public void setReturnToInner(ItemStack itemStack, boolean returnToInner) {
        if (itemStack.hasTagCompound()) {
            itemStack.getTagCompound()
                .setBoolean(NBT_RETURNTOINNER, returnToInner);
        }
    }
