...
public class Kineticator extends ConfigurableDamageIndicatedItemFluidContainer {
    ...
    public static void initInstance(ExtendedConfig<ItemConfig> eConfig) {
        if (_instance == null) _instance = new Kineticator(eConfig);
    ...
    public ItemStack onItemRightClick(ItemStack itemStack, World world, EntityPlayer player) {
        if (!ItemPowerableHelpers.onPowerableItemItemRightClick(itemStack, world, player, POWER_LEVELS, false)
            && !world.isRemote) {
    ...
    public void addInformation(ItemStack itemStack, EntityPlayer entityPlayer, List list, boolean par4) {
        ...
        L10NHelpers.addStatusInfo(
            list,
            ItemHelpers.isActivated(itemStack),
            getUnlocalizedName() + ".info" + (isRepelling(itemStack) ? ".repelling" : "") + ".attraction");
        list.add(
            EnumChatFormatting.BOLD + L10NHelpers.localize(getUnlocalizedName() + ".info.area", getArea(itemStack)));
    }
    ...
    private void kineticate(ItemStack itemStack, World world, Entity entity) {
        if (ItemHelpers.isActivated(itemStack)
            ...
            if (0 == world.getWorldTime() % KineticatorConfig.tickHoldoff) {
                ...
                List<Entity> entities = world.getEntitiesWithinAABBExcludingEntity(entity, box, new IEntitySelector() {
    ...
    protected boolean canKineticateItem(ItemStack entityItem) {
        ...
        for (String name : KineticatorConfig.kineticateBlacklist) {
            if (itemRegistry.getNameForObject(entityItem.getItem())
                .equals(name)) {
                return false;
            }
        }
