...
public abstract class Mace extends ConfigurableDamageIndicatedItemFluidContainer {
    ...
    public Mace(ExtendedConfig<ItemConfig> eConfig, int containerSize, int hitUsage, int maximumCharge, int powerLevels,
        float meleeDamage) {
            ...
            for (double pointHeight = -points; pointHeight <= points; pointHeight += 0.5F) {
                if (itemRand.nextInt(particleChance) == 0) {
                    ...
                    if (world.rand.nextInt(10) == 0) {
                        ...
                        FMLClientHandler.instance()
                            .getClient().effectRenderer.addEffect(
                                new EntityPlayerTargettedBlurFX(
                                    world,
                                    scale2,
                                    motionX,
                                    motionY,
    ...
    public void onPlayerStoppedUsing(ItemStack itemStack, World world, EntityPlayer player, int itemInUseCount) {
        ...
        int toDrain = itemUsedCount * getCapacity(itemStack)
            * (getPower(itemStack) + 1)
    ...
    public Multimap getAttributeModifiers(ItemStack itemStack) {
        ...
        multimap.put(
            ...
            new AttributeModifier(field_111210_e, "Weapon modifier", (double) this.meleeDamage, 0));
        return multimap;
    }
