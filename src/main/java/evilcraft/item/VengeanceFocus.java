...
public class VengeanceFocus extends ConfigurableItem {
    ...
    public static void initInstance(ExtendedConfig<ItemConfig> eConfig) {
        if (_instance == null) _instance = new VengeanceFocus(eConfig);
    ...
    public IIcon getIcon(ItemStack stack, int renderPass, EntityPlayer player, ItemStack usingItem, int useRemaining) {
        ...
        return iconArray[Math.min(this.iconArray.length - 1, (player.getItemInUseDuration() / 3))];
    }

    private int getItemInUseDuration(EntityPlayer player) {
        return player.isUsingItem() ? ObfuscationHelpers.getItemInUse(player)
            .getMaxItemUseDuration() - ObfuscationHelpers.getItemInUseCount(player) : 0;
    }
    ...
    public void onPlayerStoppedUsing(ItemStack itemStack, World world, EntityPlayer player, int duration) {
        if (player.worldObj.isRemote && getItemInUseDuration(player) > 6) {
            ...
            EvilCraft.proxy.playSound(
                ...
                player.posZ,
                "vengeanceBeamStop",
                0.6F + player.worldObj.rand.nextFloat() * 0.2F,
                1.0F);
        }
    }
