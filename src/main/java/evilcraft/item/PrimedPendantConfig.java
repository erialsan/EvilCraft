...
public class PrimedPendantConfig extends ItemConfig {
    ...
    @ConfigurableProperty(
        ...
        comment = "The capacity of the pendant.",
    ...
    @ConfigurableProperty(
        category = ConfigurableTypeCategory.ITEM,
        comment = "The amount of Blood to drain after one effect application.",
        isCommandable = true)
    public static int usage = 10;

    /**
     * Usage multipliers. Potion ids are first, followed by floating numbers. A number smaller than one blacklists that
     * potion.
     */
    @ConfigurableProperty(
        category = ConfigurableTypeCategory.ITEM,
    ...
    public static String[] potionMultipliers = new String[] { Potion.heal.getId() + DELIMITER + "-1",
        ...
        for (String line : config) {
            ...
            if (split.length != 2) {
                throw new IllegalArgumentException(
                    "Invalid line '" + line + "' found for " + "a Primed Pendant potion multiplier config.");
            }
            try {
                ...
                if (potionId >= Potion.potionTypes.length || Potion.potionTypes[potionId] == null) {
                    EvilCraft.log(
                        "Invalid line '" + line
                            ...
                            + split[0]
                            + " does not refer to an existing potion; skipping.");
                }
                ...
                try {
                    ...
                    EvilCraft.log(
                        "Invalid ratio '" + split[1]
                            + "' in "
                            + "a Primed Pendant potion multiplier config, using 1.0.",
                        Level.ERROR);
                }
