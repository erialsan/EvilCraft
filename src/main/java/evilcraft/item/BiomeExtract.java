...
public class BiomeExtract extends ConfigurableItem {
    ...
    public static void initInstance(ExtendedConfig<ItemConfig> eConfig) {
        if (_instance == null) _instance = new BiomeExtract(eConfig);
    ...
    public ItemStack onItemRightClick(ItemStack itemStack, World world, EntityPlayer player) {
        if (!world.isRemote && getBiome(itemStack) != null
            && !BiomeExtractConfig._instance.isUsageBlacklisted(getBiome(itemStack).biomeID)) {
    ...
    public BiomeGenBase getBiome(ItemStack itemStack) {
        if (itemStack.hasTagCompound()) {
            int biomeId = itemStack.getTagCompound()
                .getInteger(NBT_BIOMEID);
    ...
    public EnumRarity getRarity(ItemStack itemStack) {
        ...
        if (biome == null) {
            ...
            return biome.getSpawningChance() <= 0.05F ? EnumRarity.epic
                : (biome.getSpawningChance() <= 0.1F ? EnumRarity.rare : EnumRarity.uncommon);
        }
    }
}
