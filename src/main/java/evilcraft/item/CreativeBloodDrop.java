...
public class CreativeBloodDrop extends ConfigurableDamageIndicatedItemFluidContainer {
    ...
    public static void initInstance(ExtendedConfig<ItemConfig> eConfig) {
        if (_instance == null) _instance = new CreativeBloodDrop(eConfig);
    ...
    public static void updateAutoFill(ItemFluidContainer item, ItemStack itemStack, World world, Entity entity) {
        if (entity instanceof EntityPlayer && !world.isRemote && ItemHelpers.isActivated(itemStack)) {
            ...
            if (tickFluid != null && tickFluid.amount > 0) {
                ...
                if (held != null && held != itemStack
                    && held.getItem() instanceof IFluidContainerItem
                    && !player.isUsingItem()) {
                    IFluidContainerItem fluidContainer = (IFluidContainerItem) held.getItem();
                    FluidStack heldFluid = fluidContainer.getFluid(held);
                    if (/*
                         * tickFluid.amount >= MB_FILL_PERTICK Not required for creative mode filling
                         * &&
                         */(heldFluid == null || (heldFluid.isFluidEqual(tickFluid)
                        && heldFluid.amount < fluidContainer.getCapacity(held)))) {
                        int filled = fluidContainer
                            .fill(held, new FluidStack(tickFluid.getFluid(), MB_FILL_PERTICK), true);
    ...
    public boolean onItemUseFirst(ItemStack itemStack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (player.isSneaking()) {
            if (Configs.isEnabled(BloodStainedBlockConfig.class) && (BloodStainedBlock.getInstance()
                .canSetInnerBlock(block, world, x, y, z) || block == BloodStainedBlock.getInstance())) {
                BloodStainedBlock.getInstance()
                    .stainBlock(world, new Location(x, y, z), FluidContainerRegistry.BUCKET_VOLUME);
                if (world.isRemote) {
